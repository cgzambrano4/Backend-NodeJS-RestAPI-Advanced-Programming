...
exports.getRegisterById = async (req, res) => {
    ...
    try {
        const db = getDB();
        const register = await db.collection('registers').findOne({ _id: new ObjectId(id) });
        if (!register) {
            return res.status(404).json({ state: 'error', code: 404, message: 'Registro no encontrado' });
        }
        res.json({ state: 'exito', code: 200, message: 'Registro obtenido correctamente', data: register });
    } catch (err) {
        console.error(err);
        res.status(500).json({ state: 'error', code: 500, message: 'Error del servidor' });
    }
...
exports.getRegisterByIdentification = async (req, res) => {
    ...
    try {
        const db = getDB();
        const register = await db.collection('registers').findOne({ identification: identification });
        if (!register) {
            return res.status(404).json({ state: 'error', code: 404, message: 'Registro no encontrado' });
        }
        res.json({ state: 'exito', code: 200, message: 'Registro obtenido correctamente', data: register });
...
exports.updateRegisterByIdentification = async (req, res) => {
    const { identification } = req.params;
    const { phone, email, name, birthdate, id_province, address, id_gender, id_commandType, gradeNote } = req.body;
    try {
        const db = getDB();
        const updatedRegister = await db.collection('registers').findOneAndUpdate(
            { identification: identification },
            { $set: { phone, email, name, birthdate, id_province, address, id_gender, id_commandType, gradeNote } },
...
exports.createOrUpdateRegister = async (req, res) => {
    ...
    try {
        const db = getDB();
        const existingRegister = await db.collection('registers').findOne({ identification: identification });
        if (existingRegister) {
